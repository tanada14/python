...
if __name__ == "__main__":
    ...
    vehicle_sum = 4      #搬送車の総数
    vehicle_work = 0     #仕事中の搬送車
    kumitate_sum = 6     #組み立て場所の総数
    point = []           #搬送地点をリスト型で保持
    carrier = []         #搬送物をリスト型で保持
    ...
    q = deque([])        #キューの箱
    map_length = 64      #マップの縦幅
    map_side = 44        #マップの横幅
        
    #マップの作成
    f = open('map_test4-1.txt')
    ...
    #搬送地点を作成
    count = 0
    for l in open('point_test4-1.txt').readlines():
        data = l[:-1].split(' ')
        count = count+1
        in_x = int(data[0])
        in_y = int(data[1])
    ...
    #搬送車の作成
    for l in open('vehicle_position1_test4-2.txt').readlines():
        data = l[:-1].split(' ')
    ...
    for count in range(vehicle_sum):
        ...
        if((vehicle[count].state == 'move') or(vehicle[count].state == 'ready_move')):
            ...
            print(vehicle[count].vehicle_n, vehicle[count].x, vehicle[count].y, vehicle[count].state)
