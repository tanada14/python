                            ...
                            if(count != self.vehicle_n):
                                if((j == self.vehicle_x[count]) and (i == self.vehicle_y[count])):
                                    if((j == self.limit_x) and (i == self.limit_y)):
                                        ...
                                        self.dynamic_potential[i*map_side+j] = self.dynamic_potential[i*map_side+j] +1 + potential(j, i, self.limit_x, self.limit_y, 3)
                                else:
                                    if((j == self.limit_x) and (i == self.limit_y)):
                                        self.dynamic_potential[i*map_side+j] = self.dynamic_potential[i*map_side+j] +1 + 1
                                    else:
                                        self.dynamic_potential[i*map_side+j] = self.dynamic_potential[i*map_side+j] + potential(j, i, self.vehicle_x[count], self.vehicle_y[count], 3) +potential(j, i, self.limit_x, self.limit_y, 3)
...
def potential(x1, y1, x2, y2, type):
    if(type == 1):
        return -3/math.sqrt((x1-x2)**2 + (y1-y2)**2)
    elif(type == 2):
