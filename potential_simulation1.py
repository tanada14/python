...
class Vehicle:
    ...
    def __init__(self, x, y, ready_x, ready_y, goal, direction, state, vehicle_n, muki):
        ...
        self.finish_dijkstra = 0  #ダイクストラによる経路導出が終了しているか
    ...
    #ダイクストラ法による移動（移動予定のセルを選択するまで）
    def dijkstra(self):
        if(finish_dijkstra == 1):
...
def potential(x1, y1, x2, y2, type):
    if(type == 1):
        ...
        return 0.3/(math.sqrt((x1-x2)**2 + (y1-y2)**2))**2
...
if __name__ == "__main__":
    ...
    #搬送地点を作成
    count = 0
    for l in open('point_potential1.txt').readlines():
        data = l[:-1].split(' ')
        count = count+1
        in_x = int(data[0])
        in_y = int(data[1])
    ...
    #搬送車の作成
    for l in open('vehicle_position_potential1.txt').readlines():
        data = l[:-1].split(' ')
    ...
    #搬送物の生成
    count = 0
    for l in open('carrier_potential1.txt').readlines():
        data = l[:-1].split(' ')
    ...
    for time in range(10000):
        ...
        for count in range(vehicle_sum):
            if(vehicle[count].state == 'ready_move'):
                ...
                if(vehicle[count].vehicle_n == 0):   #優先度が最も高い搬送車か判定
                    vehicle[count].dijkstra()
    ...
    print('最終走行完了時間', last-carrier[0].carrier_time)