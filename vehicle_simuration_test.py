        ...
        for i in range(map_length):
            for j in range(map_side):
                if(self.static_potential[i*map_side+j] != -16):
                    if((j != x1) and (i != y1)):
                        self.static2_potential[i*map_side+j] = self.static_potential[i*map_side+j] + potential(j, i, x1, y1, 1)
                    else:
    ...
    for time in range(10):
        ...
        if(len(q) != 0):
            ...
            if(min_vehicle == -1):
                ...
                vehicle[min_vehicle].change_s_potential(point[carrier[tmp].start].out_x, point[carrier[tmp].start].out_y)
        ...
        if(time == carrier[go_carrier].carrier_time):
            ...
            if(min_vehicle == -1):
                ...
                vehicle[min_vehicle].state = 'move'
                vehicle[min_vehicle].change_s_potential(point[carrier[go_carrier].start].out_x, point[carrier[go_carrier].start].out_y)
