...
class Vehicle:
    ...
    #衝突が起きたときの動作を再現
    #移動予定場所が他の搬送車の現在地の場合、移動予定場所を現在地に変更
    def check_collision(self):
        for count in range(vehicle_sum):
            if((self.next_x == self.vehicle_x[count]) and (self.next_y == self.vehicle_y[count])):
                self.next_x = self.x
                self.next_y = self.y
            

    #停留している場合は時限ポテンシャルを定義する座標を決定
    def limit_potential(self):
        '''
        self.limit_x = self.x
        self.limit_y = self.y
        '''
    ...
    for time in range(10000):
        ...
        for count in range(vehicle_sum):
            if(vehicle[count].state == 'ready_move'):
                ...
                vehicle[count].check_collision()
                vehicle[count].limit_potential()
                vehicle[count].move()
