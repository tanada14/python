    ...
    def time_required(self):
        if(self.start == 0):
            if((self.arrival_time-self.occured_time) <= (60*self.para)):
                self.ok = 1
            print(self.arrival_time, self.occured_time, self.arrival_time-self.occured_time, 60*self.para, self.ok)
        elif(self.start == 1):
            if((self.arrival_time-self.occured_time) <= (1*self.para)):
                self.ok = 1
            print(self.arrival_time, self.occured_time, self.arrival_time-self.occured_time, 1*self.para, self.ok)
...
if __name__ == "__main__":
    ...
    for time in range(50000):
        ...
        if(time == carrier[go_carrier].occured_time):
            print(carrier[go_carrier].carrier_n, '搬送物の発生')
            #搬送要求の割り当て処理
            ...
            if(min_vehicle == -1):
                ...
                print(carrier[go_carrier].carrier_n)
                vehicle[min_vehicle].state = 'move'
                vehicle[min_vehicle].change_s_potential()
